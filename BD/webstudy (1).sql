...
--
-- Host: 127.0.0.1
-- Generation Time: May 23, 2021 at 02:30 AM
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_buscadorUsuarios` (IN `palabra` VARCHAR(100), IN `inEmail` VARCHAR(100), IN `inContra` VARCHAR(16))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_calculaProgreso` (IN `inEmail` VARCHAR(100), IN `inContra` VARCHAR(16), IN `inIdCurso` INT)  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_cargaChats` (IN `inEmail` VARCHAR(100), IN `inContra` VARCHAR(16))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_cargaComentariosCurso` (IN `inIdCurso` INT)  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_cargaCursosProfesor` (IN `inEmail` VARCHAR(100), IN `inContra` VARCHAR(16))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_cargaHistorial` (IN `INRol` VARCHAR(15), IN `INEmail` VARCHAR(100), IN `INContrasena` VARCHAR(16))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_cargaNombreCursoEvaluar` (IN `inIdCurso` INT)  BEGIN
    SELECT nombreCurso
    FROM curso
    WHERE curso.idCurso = inIdCurso;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_creaChat` (IN `inIdDelOtro` INT, IN `inEmail` VARCHAR(100), IN `inContra` VARCHAR(16))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_creaMensaje` (IN `inMensaje` VARCHAR(1000), IN `inIdChat` INT, IN `inEmail` VARCHAR(100), IN `inContra` VARCHAR(16))  BEGIN
    DECLARE idUsuario INT;
    DECLARE esUser1 BIT;
    DECLARE esUser2 BIT;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_createCategory` (IN `inNombre` VARCHAR(80), IN `inDesc` VARCHAR(1000), IN `inFoto` BLOB)  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_createComentarioCurso` (IN `inValoracion` FLOAT, IN `inContenido` VARCHAR(5000), IN `inEmail` VARCHAR(100), IN `inContra` VARCHAR(16), IN `inIdCurso` INT)  BEGIN
    DECLARE idUsuario INT;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_createCurso` (IN `inNombre` VARCHAR(80), IN `inDescripcionCorta` VARCHAR(200), IN `inDescripcion` VARCHAR(1000), IN `inCosto` FLOAT, IN `inCategoria` INT, IN `inRolUsuarioCreador` VARCHAR(15), IN `inContraUsuarioCreador` VARCHAR(16), IN `inEmailUsuarioCreador` VARCHAR(100), IN `inFoto` BLOB, IN `inVideo` BLOB)  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_createNivel` (IN `inNombre` VARCHAR(80), IN `inDesc` VARCHAR(1000), IN `inIdCurso` INT, IN `inVideo` BLOB, IN `inArchivo` BLOB)  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_estableceChat` (IN `inIdChat` INT)  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_getAlumnosDeCurso` (IN `inIdCurso` INT)  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_getCursos` ()  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_getCursosCat` (IN `inIdCat` INT)  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_getDatosCat` (IN `inIdCat` INT)  BEGIN
    SELECT C.nombreCat, C.descripcionCat, C.fotoCat
    FROM categoria AS C
    WHERE C.idCat = inIdCat;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_getDiploma` (IN `inEmail` VARCHAR(100), IN `inContra` VARCHAR(16), IN `inIdCurso` INT)  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_inscribeUsuario` (IN `inEmail` VARCHAR(100), IN `inContra` VARCHAR(16), IN `inIdCurso` INT)  BEGIN
    DECLARE idAlumno INT;
    DECLARE idProfesor INT;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_loginUsuario` (IN `INRol` VARCHAR(15), IN `INEmail` VARCHAR(100), IN `INContrasena` VARCHAR(16))  BEGIN
SELECT usuario.id,
    usuario.rol,
    usuario.nombre,
    usuario.apellidoPat,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_modificarFotoUsuario` (IN `inFoto` BLOB, IN `inRol` VARCHAR(15), IN `inEmail` VARCHAR(100), IN `inContra` VARCHAR(16))  BEGIN
    DECLARE idUser INT;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_modificarUsuario` (IN `inNombre` VARCHAR(80), IN `inApellidoPat` VARCHAR(80), IN `inApellidoMat` VARCHAR(80), IN `inContra` VARCHAR(16), IN `inEmail` VARCHAR(100), IN `inTel` VARCHAR(10), IN `inSessionRol` VARCHAR(15), IN `inSessionEmail` VARCHAR(100), IN `inSessionContra` VARCHAR(16))  BEGIN
    UPDATE usuario
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_pasarNivel` (IN `inEmail` VARCHAR(100), IN `inContra` VARCHAR(16), IN `inIdCurso` INT)  BEGIN
    DECLARE idAlumno INT;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_signupUsuario` (IN `inRol` VARCHAR(15), IN `inNombre` VARCHAR(80), IN `inApellidoPat` VARCHAR(80), IN `inApellidoMat` VARCHAR(80), IN `inContra` VARCHAR(16), IN `inEmail` VARCHAR(100), IN `inTel` VARCHAR(10))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_traeDatosCurso` (IN `inIdCurso` INT)  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_traeFotoUsuario` (IN `inRol` VARCHAR(15), IN `inEmail` VARCHAR(100), IN `inContra` VARCHAR(16))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_traeInfoCurso` (IN `inIdCurso` INT)  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_traeInfoUsuario` (IN `INRol` VARCHAR(15), IN `INEmail` VARCHAR(100), IN `INContrasena` VARCHAR(16))  BEGIN
    SELECT id, nombre, apellidoPat, apellidoMat, contrasena, email, telefono, foto
    FROM usuario
    WHERE usuario.rol = INRol AND usuario.email = INEmail AND usuario.contrasena = INContrasena;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_traeTodasCategorias` ()  BEGIN
    SELECT idCat1, nombreCat1
    FROM vw_categorias;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_verificaEstudianteEnCurso` (IN `inEmail` VARCHAR(100), IN `inContra` VARCHAR(16), IN `inIdCurso` INT)  BEGIN
    DECLARE idAlumno INT;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_verificaIdUsuarioCreador` (IN `inRol` VARCHAR(15), IN `inEmail` VARCHAR(100), IN `inContra` VARCHAR(16), IN `inIdCurso` INT)  BEGIN
...
-- Functions
--
CREATE DEFINER=`root`@`localhost` FUNCTION `fn_buscaAMCreadorMensaje` (`idUsuario` INT) RETURNS VARCHAR(80) CHARSET utf8mb4 BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `fn_buscaAPCreadorMensaje` (`idUsuario` INT) RETURNS VARCHAR(80) CHARSET utf8mb4 BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `fn_buscaContraCreadorMensaje` (`idUsuario` INT) RETURNS VARCHAR(16) CHARSET utf8mb4 BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `fn_buscaEmailCreadorMensaje` (`idUsuario` INT) RETURNS VARCHAR(100) CHARSET utf8mb4 BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `fn_buscaFotoCreadorMensaje` (`idUsuario` INT) RETURNS BLOB BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `fn_buscaNombreCreadorMensaje` (`idUsuario` INT) RETURNS VARCHAR(80) CHARSET utf8mb4 BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `fn_calculaProgreso` (`inIdEstudiante` INT, `inIdCurso` INT) RETURNS INT(11) BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `fn_calculaTotalVentasTodosCursos` (`idProfesor` INT) RETURNS FLOAT BEGIN
    DECLARE total FLOAT;
...
CREATE DEFINER=`root`@`localhost` FUNCTION `fn_calculaValoracion` (`inIdCurso` INT) RETURNS FLOAT BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `fn_calculaVentasTotales` (`inIdCurso` INT) RETURNS FLOAT BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `fn_cantidadComentariosCurso` (`inIdCurso` INT) RETURNS INT(11) BEGIN
    DECLARE cantidad INT;
...
CREATE DEFINER=`root`@`localhost` FUNCTION `fn_cantidadDeNivelesCurso` (`inIdCurso` INT) RETURNS INT(11) BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `fn_cantidadNivelesDelCurso` (`idCurso` INT) RETURNS INT(11) BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `fn_comparaBuscador` (`inPalabra` VARCHAR(100), `inVariable` VARCHAR(100)) RETURNS INT(11) BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `fn_creaChat` (`idUsuarioActual` INT, `inIdDelOtro` INT) RETURNS VARCHAR(3) CHARSET utf8mb4 BEGIN
    INSERT INTO chat(idUsuario1, idUsuario2) VALUES(idUsuarioActual, inIdDelOtro);
RETURN "ok1";
END$$

CREATE DEFINER=`root`@`localhost` FUNCTION `fn_fechaDesarrollada` (`inIdUsuario` INT, `inIdCurso` INT) RETURNS VARCHAR(24) CHARSET utf8mb4 BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `fn_fechaDiaMesAnio` (`inIdCurso` INT, `inIdUsuario` INT) RETURNS VARCHAR(10) CHARSET utf8mb4 BEGIN
    DECLARE dia VARCHAR(2);
    DECLARE mes VARCHAR(2);
    DECLARE anio VARCHAR(4);
...
CREATE DEFINER=`root`@`localhost` FUNCTION `fn_fechaDiaMesAnioFinal` (`inIdCurso` INT, `inIdUsuario` INT) RETURNS VARCHAR(23) CHARSET utf8mb4 BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `fn_fechaDiaMesAnioInicio` (`inIdCurso` INT, `inIdUsuario` INT) RETURNS VARCHAR(23) CHARSET utf8mb4 BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `fn_pasarNivel` (`inIdEstudiante` INT, `inIdCurso` INT) RETURNS INT(11) BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `fn_revisaFinalCurso` (`inIdEstudianteCursando` INT, `inIdCurso` INT) RETURNS BIT(1) BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `fn_revisaNivelFinal` (`inIdEstudianteCursando` INT, `inIdCurso` INT) RETURNS BIT(1) BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `fn_revisaUltimoNivel` (`inIdEstudianteCursando` INT, `inIdCurso` INT) RETURNS BIT(1) BEGIN
...
CREATE TABLE `estudiantescursando` (
